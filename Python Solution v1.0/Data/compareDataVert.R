...
graph <- ggplot(df, aes(x=edges,y=path_length, color=vertices)) + geom_line(data=subset(df, algorithm == "bfs"), color="grey")
...
graph <- ggplot(df, aes(x=edges,y=space, color=vertices)) + geom_jitter(size = 2, shape = 18, aes(color=algorithm))
graph <- graph + geom_smooth(data=subset(df, algorithm == "lazyrandomwalk"),se=TRUE,color="green") + geom_smooth(data=subset(df, algorithm == "randomwalk"),se=TRUE,color="blue") + geom_smooth(data=subset(df, algorithm == "bfs"),se=TRUE, color="red")
...
graph <- graph + geom_smooth(data=subset(df, algorithm == "lazyrandomwalk"),se=TRUE,color="green") + geom_smooth(data=subset(df, algorithm == "randomwalk"),se=TRUE,color="blue") + geom_smooth(data=subset(df, algorithm == "bfs"),se=TRUE, color="red")
graph <- graph + ggtitle(paste(df$vertices, "Vertex Comparison of Running Time in Miliseconds"))+ xlab("Number of Edges") + ylab("Running Time (Miliseconds/Query)")
graph <- graph + scale_colour_hue(l=30) + geom_vline(data=failureDf,color="red",size=.2,aes(xintercept=edges))
graph
#dev.off()
