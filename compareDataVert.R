...
graph <- ggplot(df, aes(x=edges,y=path_length, color=vertices)) + geom_line(data=subset(df, algorithm == "bfs"), color="grey")  + geom_line(data=subset(df, algorithm == "randomwalk"), color="grey") + geom_jitter(size = .2, aes(color=algorithm)) + geom_smooth(data=subset(df, algorithm == "randomwalk"),se=TRUE,color="green") + geom_smooth(data=subset(df, algorithm == "bfs"),se=TRUE, color="red")
...
graph <- ggplot(df, aes(x=edges,y=space, color=vertices)) + geom_jitter(size = .2, aes(color=algorithm))
...
graph <- ggplot(df, aes(x=edges,y=time, color=vertices)) + geom_jitter(size = .2, aes(color=algorithm))
graph <- graph + geom_smooth(data=subset(df, algorithm == "randomwalk"),se=TRUE,color="green") + geom_smooth(data=subset(df, algorithm == "bfs"),se=TRUE, color="red")
